...
Future<Response> buildHttpResponse(
  ...
  if (await isNetworkAvailable()) {
    ...
    if (method == HttpMethodType.POST) {
      ...
      response =
          await delete(url, headers: headers).timeout(Duration(minutes: 3),
              onTimeout: () => http.Response(
                  jsonEncode({
                    'error': {'message': 'Timeout error'}
                  }),
      ...
      response = await get(url, headers: headers).timeout(Duration(minutes: 3),
          onTimeout: () => http.Response(
              jsonEncode({
                'error': {'message': 'Timeout error'}
              }),
...
Future handleResponse(Response response,
  ...
  if (response.statusCode == 401) {
    ...
    toast('${jsonDecode(response.body)['error']['message']}');
