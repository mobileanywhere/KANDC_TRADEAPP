...
import 'package:trade/auth/auth_user_services.dart';
import 'package:trade/auth/forgot_password_dialog.dart';
import 'package:trade/auth/sign_up_screen.dart';
import 'package:trade/components/app_widgets.dart';
import 'package:trade/components/selected_item_widget.dart';
import 'package:trade/handyman/handyman_dashboard_screen.dart';
...
class SignInScreen extends StatefulWidget {
  ...
  const SignInScreen({super.key, this.isRegeneratingToken = false});
...
class _SignInScreenState extends State<SignInScreen> {
  ...
  void redirectWidget({required UserData res}) async {
    ...
    if (res.status.validate() == 1) {
      ...
      appStore.setTester(
        res.email == DEFAULT_PROVIDER_EMAIL ||
      ...
      if (res.userType.validate().trim() == USER_TYPE_PROVIDER) {
        ...
        HandymanDashboardScreen().launch(
          context,
          isNewTask: true,
          pageRouteAnimation: PageRouteAnimation.Fade,
        );
    ...
    return Scaffold(
      appBar: appBarWidget(
        ...
        systemUiOverlayStyle: SystemUiOverlayStyle(
          statusBarIconBrightness: getStatusBrightness(
            val: appStore.isDarkMode,
          ),
      ...
      body: SizedBox(
        ...
        child: Stack(
          children: [
            ...
            Observer(
              builder:
                  (_) => LoaderWidget().center().visible(appStore.isLoading),
            ),
          ],
        ),
      ),
