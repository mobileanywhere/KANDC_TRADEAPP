...
import 'package:trade/components/handyman_name_widget.dart';
...
class BasicInfoComponent extends StatefulWidget {
  ...
  final Function(bool)? onEditingHandyman;
  ...
  const BasicInfoComponent(this.flag,
      {super.key, this.customerData,
      this.handymanData,
      this.providerData,
      this.service,
      this.bookingDetail,
      this.isTechnician = false,
      this.onEditingHandyman});
...
class BasicInfoComponentState extends State<BasicInfoComponent> {
  ...
  Future<void> dShowDialog(BuildContext context) async {
      ...
      builder: (BuildContext context) {
        return StatefulBuilder(
          builder: (context, setState) {
            return AlertDialog(
              content: SearchTechnicianWidget(
                isFromBookingFragment: true,
                onSelectingTechnician: (v) {
                    ...
                    children: [
                      Row(
                        ...
                        children: [
                          ...
                          if (isUserTypeProvider && widget.isTechnician)
                            Spacer(),
                          if (isUserTypeProvider && widget.isTechnician)
                            Icon(
                              Icons.edit,
                              ...
                              // _showDialog(context);
                              widget.onEditingHandyman?.call(true);
                            })
                        ],
                      ),
                      10.height,
                      ...
                      if (widget.flag == 1)
                        ...
                        //       size: 14),
                        Text(contactNumber ?? '', style: secondaryTextStyle()),
                    ],
                  ).expand()
                ],
              ),
